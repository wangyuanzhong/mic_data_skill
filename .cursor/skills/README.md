...
1. **必须严格串行**：先完整跑完 A（步骤 0→3 全闸门，含奇异值用户确认与曲线第二阶段）→ 若还要选标正式报告则再完整跑完 B → **全部结束后**才允许写 C 宽表 / 跑 C 脚本 / 建 C 产出目录。
...
同名脚本在多个 Skill 各存一份自包含副本（不跨 Skill import，保证单 Skill 可拷走）。按是否要求行为一致分两组：

| 分组 | 文件 | 要求 |
|------|------|------|
| **等价组**（必须行为一致） | `quantize.py`（B/C）、`compose_pdf.py`（B/C）、`params_io.py`（A/C） | 公开函数对同样输入必须给出同样输出；改一份逻辑必须同步改另一份；见 `tests/test_cross_skill_script_parity.py` |
| **有意分叉**（允许不同） | `plot_style.py`（B/C） | 共用的坐标轴绘制函数（`format_hz` / `octave_centers` / `third_octave_centers` 等）走等价组规则；C 额外的曲线平滑/包络/σ 轴裁剪等函数是指向性报告专属，B 不需要，不要求同步 |

改任一等价组文件前先跑 `pytest tests/ -q`；提交前重跑确认未破坏跨 Skill 一致性。

### frontmatter 格式校验

改任一 `SKILL.md` 的 YAML frontmatter 后，跑一次官方校验（无需常驻安装）：

```bash
npx --yes skills-ref@0.1.5 validate .cursor/skills/fr-curve-golden-analysis
npx --yes skills-ref@0.1.5 validate .cursor/skills/fr-curve-measurement-report
npx --yes skills-ref@0.1.5 validate .cursor/skills/fr-curve-directivity-analysis
```
