...
5. 步骤3：跑 `run_deltas.py` → 跑 `run_freq_bins.py` → 跑 `run_consistency.py`（只传 `--params`）；脚本改写 md 分析章
6. 步骤4：出图 → 白话参数 → 组 HTML → 转 PDF → 短结
...
1. `process.xlsx`：仅步骤2写各角度宽表 sheet 可依赖 LLM；其后 sheet（`delta_*` / `freq_bins_*` / `consistency_*`）必须由脚本得到。
...
7. 闸门：0 有骨架+params → 1 有探查结论 → 1b 确认表已确认 → 2 有 params+xlsx 核对 → 3 脚本后 md 章已更新 → 4 报告已出 → 短结。
...
| `process.xlsx` | 数据处理（步骤2写各角度宽表；步骤3脚本追加 `delta_*` / `freq_bins_*` / `consistency_*`） |
...
两章都已由脚本写入后，按 [`references/fill-chat-summary.md`](references/fill-chat-summary.md) 在聊天贴短结。缺任一项 → 本步未完成。

### 硬规则

1. 必须按 3.1 → 3.2 → 3.3 → 3.4；禁止连跑完再一次性手补 md。
2. 命令行只带 `--params …`。
3. 无对应 sheet，不得填假结论。
...
脚本：把四段 note 写入 `report_sections/`、从 sheet 生成表、插图、写出 `report.html`。  
